...
\begin{document}
\tikzstyle{mlp_enc} = [rectangle, draw, fill=blue!50, text width=5em, minimum height=5em, text centered, node distance=10em]
...
\begin{frame}[fragile]{BLSTM}
	...
	\begin{frame}[fragile]{Introduction to Attention}
		\begin{center}
			\begin{tikzpicture}[scale=0.3, every node/.style={transform shape}]
				...
				\pause{}
				\node [mlp_enc, above of = ci, xshift = -8em, yshift = 3em] (decoder_tm1) {$s_{t-1}$};
				\node [above of = decoder_tm1, node distance = 5em, xshift = 8em] (ytm1) {$y_{t-1}$};
				\path [line, dotted] (decoder_tm1) to (ytm1);
				\pause{}
				\pause{}
				\node [mlp_enc, right of = decoder_tm1, xshift = 8em] (decoder_t) {$s_{t}$};
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.4, every node/.style={transform shape}]
			...
			\node [mlp_enc, right of=mlp_enc, node distance = 10em] (mlp_dec) {$MLP_{dec}$};
			\node [mlp_enc, right of=mlp_dec, node distance = 10em] (mlp_att) {$MLP_{att}$};
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.2, every node/.style={transform shape}]
			...
			\pause{}
			\node [mlp_enc, right of=h_z, node distance = 15em] (mlp_g) {$MLP_{g}$};
