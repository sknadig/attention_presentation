...
\tikzstyle{hidden} = [rectangle, draw, fill=red!50, text width=1em, minimum height=10em, minimum width=3em, text centered, node distance=10em]
\maketitle
...
\begin{frame}[fragile]{BLSTM}
	...
	\begin{frame}[fragile]{Introduction to Attention}
		\begin{center}
			\begin{tikzpicture}[scale=0.3, every node/.style={transform shape}]
				\node[mlp, node distance = 15em](LSTM_N_1){LSTM};
				\node[mlp, right of=LSTM_N_1](LSTM_N_2) {LSTM};
				\path [line] (LSTM_N_1.20) to (LSTM_N_1.20-|LSTM_N_2.west);
				\path [line] (LSTM_N_2.200) to (LSTM_N_2.-20-|LSTM_N_1.east);     
				...
				\node [hidden, above of = LSTM_N_1] (h1) {$h_{1}$};
				\node [hidden,above of = LSTM_N_2] (h2) {$h_{2}$};
				\node [hidden,above of = LSTM_N_i] (hi) {$h_{i}$};
				\node [hidden,above of = LSTM_N_Tx] (hTx) {$h_{Tx}$};
				...
				\pause{}
				\node [hidden, above of = sum1, fill=red!20] (ci) {$c_{i}$};
				
				\path [line] (sum1) to (ci.south);
				
				\pause{}
				\node [mlp, above of = ci, xshift = -8em, yshift = 3em] (decoder_tm1) {$s_{t-1}$};
				\node [above of = decoder_tm1, node distance = 5em, xshift = 8em] (ytm1) {$y_{t-1}$};
				\path [line, dotted] (decoder_tm1) to (ytm1);
				\pause{}
				\pause{}
				\node [mlp, right of = decoder_tm1, xshift = 8em] (decoder_t) {$s_{t}$};
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
			...
			\node [hidden, right of=h1, node distance = 10em, fill=red!20, minimum height = 15em] (dec_z) {$s_{i-1}$};
			...
			\node [hidden, right of=dec_z, node distance = 10em, fill=green!20, minimum height = 8em] (alpha) {w};
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.4, every node/.style={transform shape}]
			...
			\node [hidden, right of=h1, node distance = 10em, fill=red!20, minimum height = 15em, yshift=2.5em] (dec_z) {$s_{i-1}$};
			
			\node [hidden, right of=dec_z, node distance = 10em, fill=green!20, minimum height = 8em, yshift=-3.5em] (alpha) {w};
			
			\node [mlp, below of=h1, node distance = 15em] (mlp_enc) {$MLP_{enc}$};
			\node [mlp, right of=mlp_enc, node distance = 10em] (mlp_dec) {$MLP_{dec}$};
			\node [mlp, right of=mlp_dec, node distance = 10em] (mlp_att) {$MLP_{att}$};
			
			\path [line] (h1.south) to (mlp_enc.north);
			\path [line] (dec_z.south) to (mlp_dec.north);
			\path [line] (alpha.south) to (mlp_att.north);
			
			\node [hidden, below of=mlp_enc, node distance = 10em] (h_att);
			\node [hidden, below of=mlp_dec, node distance = 10em] (z_att);
			\node [hidden, below of=mlp_att, node distance = 10em] (alpha_att);
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.3, every node/.style={transform shape}]
				...
				\node [hidden, above of = mlp1, minimum height = 12em, node distance = 12em, fill=green!10] (he1) {};
				\path [line] (mlp1.north) to (he1.south);
				
				\pause{}
				\node [mlp, above of = h2, node distance = 10em] (mlp2) {$MLP_{enc}$};
				\path [line] (h2.north) to (mlp2.south);
				\node [hidden, above of = mlp2, minimum height = 12em, node distance = 12em, fill=green!10] (he2) {};
				\path [line] (mlp2.north) to (he2.south);
				
				\pause{}
				\node [mlp, above of = hi, node distance = 10em] (mlpi) {$MLP_{enc}$};
				\path [line] (hi.north) to (mlpi.south);
				\node [hidden, above of = mlpi, minimum height = 12em, node distance = 12em, fill=green!10] (hei) {};
				...
				\node [hidden, above of = mlpTx, minimum height = 12em, node distance = 12em, fill=green!10] (heTx) {};
	...
	\begin{center}
		...
		\node [mlp, fill=blue!30, above of = hei, node distance = 60em, minimum width = 10em, xshift = -10em] (sim1) {$s_{i-1}$};
		\node [mlp, fill=blue!30 , below of=sim1, minimum width = 20em] (dec_z) {decz};
		\node [mlp, below of=dec_z, node distance = 10em] (mlp_dec) {$MLP_{dec}$};
		\path [line] (dec_z) to (mlp_dec);
		\node [hidden, below of = mlp_dec, minimum height = 12em, node distance = 12em, fill=green!10] (decz_enc) {};
		...
		\node [hidden, right of = prod, minimum height = 12em, node distance = 20em, yshift=10em, fill=green!40] (h_z) {};
		...
		\pause{}
		\node [mlp, right of=e] (sm) {Softmax};
		\path [line] (e.east) to (sm.west);
		
		\pause{}
		\node [hidden, right of=sm] (alphas) {$\alpha$};
		...
		\pause{}
		\node [hidden, right of=sum_op, node distance=15em] (ci) {$c_i$};
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.2, every node/.style={transform shape}]
			...
			\pause{}
			\node [mlp, right of=h_z, node distance = 15em] (mlp_g) {$MLP_{g}$};
			\path [line, dotted] (h_z.east) to (mlp_g.west);
			
			
			\pause{}
			\node [right of=sim1, node distance = 20em, xshift=30em] (e1) {$e_{1}$};
			\path [line, dotted] (mlp_g.north) |- (e1.west);
			
			\pause{}
			\node [below of=e1, node distance = 5em] (e2) {$e_{2}$};
			\path [line, dotted] (he2.north) to [bend left=15] (prod.west);
			\path [line, dotted] (mlp_g.north) |- (e2.west);
			
			\pause{}
			\node [below of=e2, node distance = 10em] (ei) {$e_{i}$};
			\path [line, dotted] (hei.north) to [bend right=45] (prod.east);
			\path [line, dotted] (mlp_g.north) |- (ei.west);
			
			\pause{}
			\node [below of=ei, node distance = 10em] (eTx) {$e_{Tx}$};
			\path [line, dotted] (heTx.north) to [bend right=15] (prod.east);
			\path [line, dotted] (mlp_g.north) |- (eTx.west);
